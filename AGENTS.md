...
- **FCM token role mismatch (FIXED):** `NotificationService._saveTokenToFirestore` checks `role == 'job_seeker'` but `normalizeRole()` in `StorageService` produces `'jobSeeker'` (camelCase). Was: job-seeker FCM tokens NOT saved to `jobSeekers/{uid}` → push notifications silently broken for job seekers. **Fix:** Both comparisons now use `AppUserRole.jobSeeker.value` / `AppUserRole.company.value` at `notification_service.dart:89,94`.
- **Profile logout missing FCM cleanup (FIXED):** `profile_controller.dart:logout()` only called `_auth.signOut()` — no `deleteToken()`, no `FieldValue.delete()` from Firestore, no `clearAuthSession()`. **Fix:** now deletes token from role collection + `users/{uid}`, calls `deleteToken()`, and clears session, matching the chat-view logout pattern.
